...
public class Debug {
    ...
    private boolean validateNode(AugmentedNode<ILineSegment> n) {
        ...
        if (n.value() != null) {
            if (!min.equals(n.value())) {
                error(n);
                return false;
            }
            if (!max.equals(n.value())) {
                error(n);
                return false;
            }
            return true;
        ...
        if (n.left() != null) {
            if (!min.equals(n.left().min)) {
                error(n);
                return false;
            }
        }
        if (n.right() != null) {
            if (!max.equals(n.right().max)) {
                error(n);
                return false;
            }
        }
    ...
    private StringBuilder shortString(ILineSegment ils) {
        StringBuilder sb = new StringBuilder("<");
        sb.append(left4(ils.getStart().getX()));
        sb.append(",");
        sb.append(left4(ils.getStart().getY()));
        sb.append(",");
        sb.append(left4(ils.getEnd().getX()));
        sb.append(",");
        sb.append(left4(ils.getEnd().getY()));
        sb.append(">");
        return sb;
    }
